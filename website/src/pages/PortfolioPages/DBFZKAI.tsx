...
with the balance state of several characters, and started writing up some \
quick ideas regarding how changes could make them both more enjoyable and \
balanced in the base roster. Eventually, this morphed into full-fledged \
system overhaul for every character and in-depth changes to all their \
mechanics.'
...
\n\n\
The lack of new animation frames and removing assists necessitated adding \
affordances to character actions without fundamentally altering them, and \
adding new baseline mechanics to not remove depth from the original game. Without \
assists, many characters lost fundamental ways of creating openings in the \
opponent, or lost the ability to control space reliably.\n\nWithout the ability \
to create new animation frames, I could not create new character actions ex-nihilo; \
every new action or altered action had to be derived from other moves that already \
were present. This creates its own problems, as character normal & special attacks \
in the base game were designed to be significantly larger in order to catch the opponent \
over larger portions of screen space, and make \'tightening up\' the design space \
rather difficult.'
const challenges= "Challenges for the new ruleset"


const header2 = "Solution Through Innovation"
const paragraph3 = 'My Design philosophy for solving the majority of problems was \
to use multi-faceted solutions; create elementing within the games’ ruleset that \
solve more than one problem at once. It reduced my own scope overhead, and meant \
I had to think of novel solutions to bespoke fighting game problems. \n\nBelow are \
some of the solutions I developed:'

const header3="Design Results"
const paragraph4='Kai Cancels ended up being the most fun new aspect to the mod; \
they revitalized ranged gameplay and provided move character uniqueness by further \
delineating move functionality for each character, while also greatly expanding the \
combo space of each character. Now Ki Specials are contextualized with the Normal \
Specials a character has as well, effectively recontextualizing all Special moves on \
every character. Several of the players I playtested with commented it should be in \
the base game.\n\n Faster normals shortened combo duration, blockstrings, and pressure \
sequences, but didn\'t change the pace of gameplay where neither player is advantaged. \
This was an unintended effect, but was not outside of the original design goals, so was \
not addressed.\n\nRubberband Dashes were recieved strangely, and many players commented \
on how the entire game felt stranger when they were introduced. The design hypothesis \
for this is that all of the playtesters were native Dragon Ball FighterZ players, and \
thus had an inbuilt context for the game. Making changes to the fundamental aspects \
was outside of their frame of reference, and they were unsure how to factor it into \
behavioral changes during gameplay. More testing with a purely new fighting game engine \
would have led to more undiluted results.'

const header4 = "Post Mortem"
const paragraph5 = 'I ended up having to shelve this project indefinitely. At the time, \
the modding tools for DBFZ were rudimentary, and I was unwilling to dig into executable \
hacking to try and further the tools necessary for them. Several of the changes I made \
to characters in Kai would show up in whole or in part in the game in later seasons. SSJ \
Vegeta\'s ability to cancel his Consecutive Ki blasts, as well as the ability to combo \
into Vegeta Blue\'s hitgrabs in midair being the most notable examples. I learned a lot \
